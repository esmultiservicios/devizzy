<?php
	...
	if($result->num_rows>0){
		...
		ob_start();
		include(dirname('__FILE__').'/nominaComprobante.php');
		$html = ob_get_clean();
		...
		file_put_contents(dirname('__FILE__').'/nomina/nominaComprobante_'.$no_factura.'.pdf', $dompdf->output());
		
		// Output the generated PDF to Browser
		$dompdf->stream('nominaComprobante_'.$no_factura.'.pdf',array('Attachment'=>0));
