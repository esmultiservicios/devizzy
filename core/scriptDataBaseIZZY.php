...
$users_id = 1;
$privilegio_id = 1;//ADMINISTRADOR
$tipo_user_id = 1;//ADMINISTRADOR
$username = "";
...
if (empty($resultadoServercustomers)) {
  ...
  INSERT INTO `clientes` (`clientes_id`, `nombre`, `rtn`, `fecha`, `departamentos_id`, `municipios_id`, `localidad`, `telefono`, `correo`, `estado`, `colaboradores_id`, `fecha_registro`) VALUES
  (1, 'Consumidor Final', '999999999', CAST(NOW() AS DATE), 18, 295, '.', '0', 'alguien@algo.com', 1, 1, NOW());
  ...
  INSERT INTO `users`(`users_id`, `colaboradores_id`, `privilegio_id`, `username`, `password`, `email`, `tipo_user_id`, `estado`, `fecha_registro`, `empresa_id`) VALUES ('$users_id','$colaboradores_id','$privilegio_id','$username','$contraseña_generada','$correo','$tipo_user_id','$estado','$fecha_registro','$empresa_id');
  ...
  $valores = [$database->obtenerCorrelativo($tabla, $campoCorrelativo), $clientes_id, $databaseCliente, $validar, $sistema_id, $planes_id, "1"];
...
function connectionScript($databaseCliente){
  $conexion = new mysqli(SERVER, USER, PASS, $databaseCliente);
