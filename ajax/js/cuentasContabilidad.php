...
var listar_cuentas_contabilidad = function(){	
	...
	var table_cuentas_contabilidad  = $("#dataTableCuentasContabilidad").DataTable({
		...
		"columnDefs": [
		  ...
		  { width: "17.11%", targets: 1 },
		  { width: "15.11%", targets: 2 },
		  { width: "11.11%", targets: 3 },
		  { width: "11.11%", targets: 4 },
		  { width: "13.11%", targets: 5 },
	...
	$(tbody).on("click", "button.table_editar", function(){
		...
		$.ajax({
			...
			success: function(registro){
				...
				$('#formCuentasContables').attr({ 'data-form': 'update' });
				$('#formCuentasContables').attr({ 'action': '<?php echo SERVERURL;?>ajax/modificarCuentaContabilidadAjax.php' });
...
var eliminar_cuentas_contabilidad_dataTable = function(tbody, table){
	...
	$(tbody).on("click", "button.table_eliminar", function(){
		...
		$.ajax({
			...
			success: function(registro){
				var valores = eval(registro);
				$('#formCuentasContables').attr({ 'data-form': 'delete' });
				$('#formCuentasContables').attr({ 'action': '<?php echo SERVERURL;?>ajax/eliminarCuentaContabilidadAjax.php' });
				$('#formCuentasContables')[0].reset();
				$('#reg_cuentas').hide();
				$('#edi_cuentas').hide();
				$('#delete_cuentas').show();
...
function modal_cuentas_contables(){
	$('#formCuentasContables').attr({ 'data-form': 'save' });
	$('#formCuentasContables').attr({ 'action': '<?php echo SERVERURL;?>ajax/addCuentasContablesAjax.php' });
	$('#formCuentasContables')[0].reset();
	$('#reg_cuentas').show();
	$('#edi_cuentas').hide();
	$('#delete_cuentas').hide();
